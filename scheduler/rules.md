...
- 输出：`output/daily/us-close-YYYY-MM-DD.md` + `output/content/tweet-YYYY-MM-DD-us-close.md`（日报后立即按 `templates/close-tweet.md`）
- 飞书标题：`美股收盘推文 YYYY-MM-DD`（`feishu_send.py … tweet-…-us-close.md --section 主贴 --also us-close-….md`；**不推**日报整文；先 `merge_to_main.sh`）
...
- **正文只保留连续 §0–§8**（**§0**=一句话总结 + **进攻四条件**一行结果 + **防守四条件**一行结果 + **账户重心**；**§8**=**分层回顾**四块（指数与均线/结构与驱动/赚钱效应与量能/结论）+ 阶段 + 操作倾向 + 信号；§1 含状态评分 + 交易结论；**赚钱效应行内含**全市场涨跌家数 + **涨跌中位数 vs 上证**（`market_breadth.py`）+ **成交额前100涨:跌**（仅家数，`top100_amount.py`）+ 涨跌停/连板；**禁止**另开活跃成交/前100行业分布专节；§4=**资金与板块共振**：4.1 行业线资金×涨跌 / 4.2 概念线资金×涨跌 / 4.3 交叉轮动；§5=持仓+近7日负面；§6=明日应对五要点；§7=用户池；§8=结论）；禁止跳号；禁止再单开「今日资金面 / 板块表现 / 主题概念 / 持仓负面 / 轮动」专章
- **数据源优先级**：扶摇 MCP 管 **指数+持仓 snapshot**；涨跌停/东财板块涨跌 = `limit_pool_summary.py`（家数=沪深A收盘封死，含ST，不含新股首日/北交所/触及） / `board_top.py`；量能 = `market_turnover.py`（**沪深京合计**，含北交所）；均线·空间·周期 = 腾讯/scripts。硬规则：historical 不作主源；搜索带年；**禁止 THS catalog / 涨跌停全池 JSON**。失败 → 东财 + 本地 tools
- 执行：市场状态 **状态评分** + **§1 交易结论**（评分｜状态｜策略一行；操作提示 1 句；账户重心 1 句：好→国元GY+银河YH进攻 / 不好→华泰HT+银河YH防守短线）+ **§2 表后固定三段**（指数结构 / 上证技术位置 / 市场宽度）+ **§3 盘中固定三段**（盘面演变：早/午/午后/尾盘；核心驱动 3～4 条标签；盘中结论 2～3 句；禁止箭头时间线）+ **§4 资金与板块共振** + **持仓短评含负面 + 个股势能单表6行（§5；ETF跳过）** + **明日应对五要点（§6；条内必须分行，禁止超长单行）** + **用户池五选一（§7）** + 结论；仓位建议可参考 `data/raw/notes/`，**冲突以 my-soul 为准**
