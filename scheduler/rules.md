...
- **正文只保留连续 §0–§8**（§4=**资金与板块共振**：4.1 行业线资金×涨跌 / 4.2 概念线资金×涨跌 / 4.3 交叉轮动；§5=持仓+近7日负面；§6=明日应对五要点；§7=用户池；§8=结论）；禁止跳号；禁止再单开「今日资金面 / 板块表现 / 主题概念 / 持仓负面 / 轮动」专章
- **数据源优先级**：**扶摇 MCP（P0）** → **同花顺网页 + 东财（P1）** → 其他/本地 tools（P2）。扶摇挂了/鉴权失败/Automations 无 MCP → **立刻回退**，禁止空等；§4 东财行业·概念资金、东财行业/概念涨跌侧、筹码 CYQ、个股负面仍走东财/网页/脚本（扶摇暂无）；**§1 全市场涨跌家数必须** `tools/market_breadth.py`（东财涨跌分布；禁止全市场分页硬扫）
- 执行：市场状态评分 + **§1 操作提示 1 句** + **账户重心 1 句**（好：国元GY+银河YH进攻 / 不好：华泰HT+银河YH防守短线）+ 大盘/盘中 + **§4 资金与板块共振** + **持仓短评含负面 + 势能单表6行（§5）** + **明日应对五要点（§6）** + **用户池四选一（§7）** + 结论；仓位建议可参考 `data/raw/notes/`，**冲突以 my-soul 为准**
- **持仓（§5）**：势能 **同一表 6 行**（趋势/资金·主力/板块/空间/周期势/筹码势）；空间/周期势优先扶摇日线，失败再用 `stock_ma_levels.py` / `stock_period_chg.py`；筹码势汇总（`stock_cyq_summary.py`：东财日K→腾讯换手兜底）；全节末尾 **1 句**`今日对照总纲：`；§6 持仓态度须带 **半句空间/盈亏比**；**不覆盖** my-soul 止损纪律；禁止另开势能/总纲专章
