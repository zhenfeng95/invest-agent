...
- **数据源优先级**：**扶摇 MCP（P0）** → **同花顺网页 + 东财（P1）** → 其他/本地 tools（P2）。扶摇挂了/鉴权失败/Automations 无 MCP → **立刻回退**，禁止空等；§4 东财行业资金、§5 东财行业侧、筹码 CYQ、个股负面仍走东财/网页/脚本（扶摇暂无）；**§1 全市场涨跌家数必须** `tools/market_breadth.py`（东财涨跌分布；禁止全市场分页硬扫）
- 执行：市场状态评分 + **§1 操作提示 1 句** + **账户重心 1 句**（好：国元GY+银河YH进攻 / 不好：华泰HT+银河YH防守短线）+ 大盘/盘中 + 资金面 + 板块共振/分歧 + **轮动（§5.3）** + 概念 + **持仓短评含负面 + 势能单表6行（§7）** + **明日应对五要点（§8）** + **用户池四选一（§9）** + 结论；仓位建议可参考 `data/raw/notes/`，**冲突以 my-soul 为准**
- **持仓（§7）**：势能 **同一表 6 行**（趋势/资金·主力/板块/空间/周期势/筹码势）；空间/周期势优先扶摇日线，失败再用 `stock_ma_levels.py` / `stock_period_chg.py`；筹码势汇总（`stock_cyq_summary.py`：东财日K→腾讯换手兜底）；全节末尾 **1 句**`今日对照总纲：`；§8 持仓态度须带 **半句空间/盈亏比**；**不覆盖** my-soul 止损纪律；禁止另开势能/总纲专章
...
- **板块（§5）**：同花顺侧优先扶摇 THS 指数，失败→同花顺网页；东财侧仍东财；正文出共振 + 分歧（5.1/5.2）+ **轮动解读（5.3）**；禁止贴两侧完整 TOP5；禁止再单开轮动章
...
- 说明：精简版控成本；用户池分析耗时随池大小变化；改数据源后须 **重贴** Automations Instructions
