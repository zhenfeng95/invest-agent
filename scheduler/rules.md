...
- 执行：市场状态评分 + **§1 操作提示 1 句** + **账户重心 1 句**（好：国元GY+银河YH进攻 / 不好：华泰HT+银河YH防守短线）+ **§2 表后固定三段**（指数结构 / 上证技术位置 / 市场宽度）+ **§3 盘中固定三段**（盘面演变：早/午/午后/尾盘；核心驱动 3～4 条标签；盘中结论 2～3 句；禁止箭头时间线）+ **§4 资金与板块共振** + **持仓短评含负面 + 势能单表6行（§5）** + **明日应对五要点（§6）** + **用户池四选一（§7）** + 结论；仓位建议可参考 `data/raw/notes/`，**冲突以 my-soul 为准**
