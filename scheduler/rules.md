...
- 执行：市场状态评分 + 大盘/盘中 + 资金面 + 板块共振/分歧 + **轮动（§5.3）** + 概念 + **持仓短评含负面 + 势能必评4项（§7）** + **明日应对五要点（§8）** + **用户池四选一（§9）** + 结论；仓位建议可参考 `data/raw/notes/`，**冲突以 my-soul 为准**
- **持仓（§7）**：除短评与近7日负面外，每只 A 股持仓须做框架势能 **必评 4 项**（趋势 / 资金·主力合并 / 板块 / 空间）；周期·风控·情绪·节奏·筹码为选填；筹码无可靠数据禁止猜；**不覆盖** my-soul 止损纪律；禁止另开势能专章
