...
- **数据源优先级**：**扶摇 MCP（P0）** → **同花顺网页 + 东财（P1）** → 其他/本地 tools（P2）。扶摇挂了/鉴权失败/Automations 无 MCP → **立刻回退**，禁止空等；§4 东财行业·概念资金、东财行业/概念涨跌侧、筹码 CYQ、个股负面仍走东财/网页/脚本（扶摇暂无）；**§1 全市场涨跌家数必须** `tools/market_breadth.py`（东财涨跌分布；禁止全市场分页硬扫）；**§1 成交额前100必须** `tools/top100_amount.py`（东财一页；禁止再拉行业主力净额）
