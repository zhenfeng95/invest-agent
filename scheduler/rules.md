...
- **数据源优先级**：扶摇 MCP 管 **指数+持仓 snapshot**；涨跌停/东财板块涨跌 = `limit_pool_summary.py`（家数=同花顺/东财客户端口径） / `board_top.py`；量能 = `market_turnover.py`（**沪深京合计**，含北交所）；均线·空间·周期 = 腾讯/scripts。硬规则：historical 不作主源；搜索带年；**禁止 THS catalog / 涨跌停全池 JSON**。失败 → 东财 + 本地 tools
