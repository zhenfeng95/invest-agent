...
- 执行：市场状态评分 + **§1 操作提示 1 句**（稳：行业主线+买点可激进+小波段 / 震荡：放弃大盘+强势题材+高抛低吸 / 下跌轻仓观望）+ 大盘/盘中 + 资金面 + 板块共振/分歧 + **轮动（§5.3）** + 概念 + **持仓短评含负面 + 势能单表6行（§7）** + **明日应对五要点（§8）** + **用户池四选一（§9）** + 结论；仓位建议可参考 `data/raw/notes/`，**冲突以 my-soul 为准**
- **持仓（§7）**：势能 **同一表 6 行**（趋势/资金·主力/板块/空间/周期势/筹码势）；空间势 MA60/120/250（`stock_ma_levels.py`）；筹码势汇总（`stock_cyq_summary.py`）；全节末尾 **1 句**`今日对照总纲：`；§8 持仓态度须带 **半句空间/盈亏比**；**不覆盖** my-soul 止损纪律；禁止另开势能/总纲专章
