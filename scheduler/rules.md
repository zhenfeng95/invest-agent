...
- 执行：市场状态评分 + 大盘/盘中 + 资金面 + 板块共振/分歧 + **轮动（§5.3）** + 概念 + **持仓短评含负面 + 势能必评4项 + 空间势下周期势一句（§7）** + **明日应对五要点（§8）** + **用户池四选一（§9）** + 结论；仓位建议可参考 `data/raw/notes/`，**冲突以 my-soul 为准**
- **持仓（§7）**：除短评、近7日负面、势能必评4项、**空间势行下周期势一句总结（3/5/10/20/60/YTD 斜杠串一句，禁止分列表格）**外，全节末尾 **1 句**`今日对照总纲：`；§8 持仓态度须带 **半句空间/盈亏比**；**不覆盖** my-soul 止损纪律；禁止另开势能/总纲专章
