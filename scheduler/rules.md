...
- **数据源优先级**：**扶摇 MCP（P0）** → **同花顺网页 + 东财（P1）** → 其他/本地 tools（P2）。扶摇挂了/鉴权失败/Automations 无 MCP → **立刻回退**，禁止空等；§4 东财行业资金、§5 东财行业侧、筹码 CYQ、个股负面仍走东财/网页/脚本（扶摇暂无）
...
- **持仓（§7）**：势能 **同一表 6 行**（趋势/资金·主力/板块/空间/周期势/筹码势）；空间/周期势优先扶摇日线，失败再用 `stock_ma_levels.py` / `stock_period_chg.py`；筹码势汇总（`stock_cyq_summary.py`）；全节末尾 **1 句**`今日对照总纲：`；§8 持仓态度须带 **半句空间/盈亏比**；**不覆盖** my-soul 止损纪律；禁止另开势能/总纲专章
