...
- 执行：市场状态评分 + 大盘/盘中/资金面/板块/题材/宽度/技术 + 涨停情绪 + 政策外盘 + 持仓/负面 + 盘后要闻 + 明日应对（含 **§15.6 二筛个股备查**）/风险/结论；仓位建议可参考 `data/raw/notes/`，**冲突以 my-soul 为准**
- **选股池步骤（必做）**：先跑 `tools/mtd_screener.py`（公式一层落盘 + 二筛打印；**不读**用户池、**不写** `*-buysetup`）→ `mtd-screener-*.csv` + 日报 **§15.6**；**不写荐股、不代下单**
...
- 输出：`output/daily/ashare-close-YYYY-MM-DD.md`（另存 screener **仅公式一层**一并 commit）
- 飞书标题：`A股收盘日报 YYYY-MM-DD`（`feishu_send.py` 推完整正文；先 `merge_to_main.sh`）
- 说明：篇幅与搜索量高；成本明显高于原盘前任务；选股脚本另耗网络与时间（约 2–4 分钟）
