...
- 执行：市场状态评分 + 大盘/盘中/资金面/板块/题材/宽度/技术 + 涨停情绪 + 政策外盘 + 持仓/负面 + 盘后要闻 + 明日应对（含 **§15.6 用户池四选一全量分析**）/风险/结论；仓位建议可参考 `data/raw/notes/`，**冲突以 my-soul 为准**
- **选股分析（必做）**：读 `data/raw/screener/pool-latest.csv`（或用户指定文件）→ `tools/mtd_screener.py` 全量四选一 → stdout 命中/未命中两表写入日报 **§15.6**；**不跑** `--formula`、**不写** buysetup 文件；未交池则 §15.6 注明；**不写荐股、不代下单**
- **板块口径（全文）**：**东财 ∥ 同花顺** 并列主源（行业涨跌/资金/轮动/观察均须标口径）；题材可用双源概念；禁止申万/数据宝/界面顶替；冲突时两表都留 + 共振/分歧一句
- **板块（第5节）**：东财领涨/领跌各 TOP5 **且** 同花顺领涨/领跌各 TOP5；一侧抓不到写未获取
- **轮动（第12节）**：基于 §4/§5 双源；必写双源对照
...
- 说明：篇幅与搜索量高；成本明显高于原盘前任务；用户池分析耗时随池大小变化（通常远快于全市场公式）
