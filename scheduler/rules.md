...
- **正文只保留连续 §0–§8**（§1 含状态评分 + 交易结论 + **成交额前100涨跌与东财行业分布**；§4=**资金与板块共振**：4.1 行业线资金×涨跌 / 4.2 概念线资金×涨跌 / 4.3 交叉轮动；§5=持仓+近7日负面；§6=明日应对五要点；§7=用户池；§8=结论）；禁止跳号；禁止再单开「今日资金面 / 板块表现 / 主题概念 / 持仓负面 / 轮动」专章
- **数据源优先级**：**扶摇 MCP（P0）** 管当日指数 snapshot / 涨跌停 / THS / 个股价量；**上证均线与 §5 空间/周期势主源改为腾讯日 K / `stock_ma_levels.py` / `stock_period_chg.py`**（扶摇 historical 禁作主源）。**昨量**：只读昨报 §1 量能句（勿通读）。**搜索/historical**：必须带年份并与昨报对账，异常跳变作废。扶摇挂了 → **立刻回退**同花顺网页 + 东财 + 本地 tools；§4 东财资金/涨跌、筹码 CYQ、**成交额前100**、涨跌家数仍走备用脚本。禁止因 MCP 挂掉而停写日报；正文标明实际来源
- 执行：市场状态 **状态评分** + **§1 交易结论**（评分｜状态｜策略一行；操作提示 1 句；账户重心 1 句：好→国元GY+银河YH进攻 / 不好→华泰HT+银河YH防守短线）+ **§1 活跃成交前100**（涨跌家数 + 涨/跌东财行业分布）+ **§2 表后固定三段**（指数结构 / 上证技术位置 / 市场宽度）+ **§3 盘中固定三段**（盘面演变：早/午/午后/尾盘；核心驱动 3～4 条标签；盘中结论 2～3 句；禁止箭头时间线）+ **§4 资金与板块共振** + **持仓短评含负面 + 个股势能单表6行（§5；ETF跳过）** + **明日应对五要点（§6；条内必须分行，禁止超长单行）** + **用户池四选一（§7）** + 结论；仓位建议可参考 `data/raw/notes/`，**冲突以 my-soul 为准**
